...
public class AnomalyDetector {
    ...
    public List<Map<String, Object>> detectAnomalies() {
        ...
        Map<String, Double> averages = txs.stream()
                .filter(t -> t.getInventoryItem() != null && t.getInventoryItem().getSku() != null)
                .collect(Collectors.groupingBy(
                        t -> t.getInventoryItem().getSku(),
                        Collectors.averagingDouble(t -> t.getQuantity() != null ? t.getQuantity() : 0.0)
                ));

        for (StockTransaction tx : txs) {
            if (tx.getInventoryItem() == null) continue;
            ...
            if (tx.getQuantity() != null && avg > 0 && tx.getQuantity() > (avg * 4)) {
                score += 0.5;
            ...
            if (score > 0.2) {
                ...
                record.put("timestamp", tx.getCreatedAt() != null ? tx.getCreatedAt().toString() : "");
                record.put("quantity", tx.getQuantity() != null ? tx.getQuantity() : 0);
